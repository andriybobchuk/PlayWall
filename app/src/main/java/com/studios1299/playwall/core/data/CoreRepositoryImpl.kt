...
class FirebaseCoreRepositoryImpl(
    ...
    override suspend fun reportWallpaper(wallpaperId: Int?, badBoyId: Int): SmartResult<Unit> {
        return try {
            val result = performAuthRequest { token ->
                RetrofitClient.wallpaperApi.report(token, ReportRequest(wallpaperId, badBoyId))
            }
            if (result is SmartResult.Success) {
    ...
    override suspend fun debugClearCheckInShit() {
        Preferences.setConsecutiveDays(0)
        Preferences.setLastCheckInDate("2024-10-12")
        Preferences.setDevilsCount(0)
    }

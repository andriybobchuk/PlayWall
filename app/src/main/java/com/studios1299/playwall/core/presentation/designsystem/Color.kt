package com.studios1299.playwall.core.presentation.designsystem

import androidx.compose.ui.graphics.Color

// Light theme
val GRAY200 = Color(0xFFEEEFF4) // Backgrounds
val GRAY600 = Color.Gray // Grey texts
val GRAY800 = Color.Black // Black texts
val BLUE200 = Color(0xFFCEF0F8)

// Dark theme
val NAVY800 = Color(0xFF070F2B)
val NAVY600 = Color(0xFF1B1A55)
val NAVY500 = Color(0xFF1a1969)
val NAVY400 = Color(0xFF535C91)
val NAVY300 = Color(0xFF666488) // only for loading images backgrounds
val NAVY200 = Color(0xFF9290C3)

// Other:
val ERROR_RED = Color.Red
val SUCCESS_GREEN = Color.Green

val DIAMONDS_SCREEN_PANEL = Color(0xFF7a30e3)

// Light orange theme
val YELLOW100 = Color(0xFFf7e6ba)


// ZEDGE THEME
// Accent
val ZEDGE_PURPLE = Color(0xFF6A39DD)
val ZEDGE_MAGENTA = Color(0xFFFF006E)
val ZEDGE_PINK = Color(0xFFE6ACCF)
val ZEDGE_LIGHT_PURPLE = Color(0xFF7A88FF)

// Background
val ZEDGE_BLACK = Color(0xFF141414)
val ZEDGE_DARK = Color(0xFF1C1C1C)
val ZEDGE_GRAY = Color(0xFFA1A1A1)
val ZEDGE_WHITE = Color(0xFFF0F0F0)


// Dark orange theme
// Dark theme
val YELLOW800 = Color(0xFF171103)
val YELLOW600 = Color(0xFFcf9c17)
val YELLOW500 = Color(0xFFe8b530)
val YELLOW400 = Color(0xFFedc55e)
val YELLOW300 = Color(0xFFf2d68c) // only for loading images backgrounds
val YELLOW200 = Color(0xFFf7e6ba)